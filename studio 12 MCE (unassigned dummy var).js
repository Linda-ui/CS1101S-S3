...
//
// evaluation
//studio S12 q1: Function eval_sequence: In JavaScript, the function
//declarations that appear anywhere in a statement sequence of a block are automatically
//moved to the beginning of the block.

//studio S12 q2: Add a dummy value to detect unassigned(during evaluation) and throw an error
//() => anything or pair,list or any object with an identity that exist within
//MCE itself
...
function eval_sequence(stmts, env) { 
    if (is_empty_sequence(stmts)) {
        ...
        //put all func declar in the front, append rest
        
        const func = filter(is_function_declaration, stmts);
        
        const rest = filter(x => !is_function_declaration(x), stmts);
        
        const newstmts = append(func, rest);
        
        const ignore = evaluate(first_statement(newstmts), env);
        return eval_sequence(rest_statements(newstmts), env); 
    } 
}
